...
# streamlit_app.py - CORRECTED FULL VERSION WITH PIPE-VALVE MAPPING
...
def propagate_pressure(network, pressure_sources, system_name):
    pressurized_nodes = set(f"pipe_{s-1}" for s in pressure_sources)
    changed = True
    while changed:
        changed = False
                ...
                if node_data["type"]=="pipe":
                    for valve_tag in node_data.get("connected_valves", []):
                        if valve_tag not in pressurized_nodes:
                            vdata = network[valve_tag]
                            if vdata["is_special"]:
                                if vdata["is_open"]: pressurized_nodes.add(valve_tag); changed=True
                            else: pressurized_nodes.add(valve_tag); changed=True
                elif node_data["type"]=="valve":
                    if node_data["is_open"]:
                        for p_idx in node_data.get("connected_pipes", []):
                            pid = f"pipe_{p_idx}"
                            if pid not in pressurized_nodes: pressurized_nodes.add(pid); changed=True
    return pressurized_nodes

def get_pipe_pressure_status(pipe_index, pressurized_nodes):
    pid = f"pipe_{pipe_index}"
...
def run_simulation(system_name):
    ...
    with st.sidebar:
        ...
        # Keep your calibration/edit UI here (omitted for brevity)
